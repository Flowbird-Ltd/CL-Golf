    ...
    def _compute_can_be_created(self):
        for rec in self:
            ...
            for eachattline in rec.product_attribute_ids:
                if eachattline.value_id and eachattline.value_id.child_attval_ids:
                    child_att_ids = eachattline.value_id.child_attval_ids.mapped('attribute_id')
                    for eca_id in child_att_ids:
                        line = rec.product_attribute_ids.filtered(lambda l: l.attribute_id.id == eca_id.id)
                        if line:
                            related_attval_ids = eachattline.value_id.child_attval_ids.filtered(lambda l: l.attribute_id.id == eca_id.id)
                            ...
                            # now find common
                            list1_as_set = set(related_attval_ids.ids)
                            intersection = list1_as_set.intersection(attributeval_ids.ids)
                            intersection_as_list = list(intersection)
                            line.possible_value_ids = [(6, 0, intersection_as_list)]
                        else:
                            eachattline._compute_possible_value_ids()
            for eachattline in rec.product_attribute_ids:
                if eachattline.value_id and (eachattline.value_id.id not in eachattline.possible_value_ids.ids):
                    eachattline.value_id = False
